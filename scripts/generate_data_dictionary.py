...
TABLES: list[dict] = [
    ...
    },
    {
        ...
        "caminho": "Sem caminho fixo em config/settings.py ainda -- não existe BronzeWriter.write_ugc_mentions",
        "grao": "Uma linha por post de TERCEIROS que marca/menciona o perfil do governador (nível 'Creating' do COBRA), por execução.",
        "modo_escrita": "append (planejado)",
        "escrito_por": "Nenhum -- schema definido em src/schemas_delta.py para a issue #93, mas nenhum writer de produção grava esta tabela ainda",
    ...
    },
    {
        ...
        "grao": "Uma linha por governador, deduplicada por `inputUrl`.",
        "modo_escrita": "overwrite",
        ...
        "notas": "`reference/governadores.xlsx` é mantido manualmente, fora do pipeline (ADR 0013) -- esta é a ÚNICA leitura de Excel que resta no projeto, e é de configuração, não de dado coletado.",
    },
    {
        "camada": "Silver",
        "tabela": "ugc_mentions (schema definido, ainda não materializado)",
        "schema": sd.SILVER_UGC_MENTIONS_SCHEMA,
        "caminho": "Sem caminho fixo em config/settings.py ainda",
        "grao": "Uma linha por post de UGC, com `governor_username` já resolvido e `authorUsername` já normalizado.",
        "modo_escrita": "overwrite (planejado)",
        "escrito_por": "src/features/silver/ugc_mention_cleaner.py (UgcMentionCleaner) -- testado com dado sintético, sem chamador de produção ainda",
        "lido_por": "src/features/gold/ugc_mentions_aggregator.py (testado com dado sintético)",
        "status": "PILOTO -- mesma ressalva da Bronze ugc_mentions",
        "adr": "ADR 0020 Ficha 8 / issue #93",
        "descricao": "UGC de terceiros já limpo: `governor_username` derivado de `mentions` cruzado com a lista de usernames conhecidos; `authorUsername` normaliza `authorUsername`/`ownerUsername` brutos num só campo.",
        "notas": "`id`/`shortCode` são nullable aqui (ao contrário do padrão NOT NULL de profiles/posts/reels) porque o piloto ainda não confirmou presença garantida de `id` no actor escolhido -- o cleaner descarta linha sem os dois.",
    },
    # ----------------------------- GOLD ----------------------------------
    {
        "camada": "Gold",
    ...
    },
    {
        ...
        "lido_por": "pages/05_funil.py (nível 'Criar' do COBRA -- volume de discurso oficial)",
        "status": "Produção",
    ...
    },
    {
        ...
        "lido_por": "pages/02_insights.py; pages/05_funil.py (nível 'Consumir' do COBRA)",
        "status": "Produção",
    ...
    },
    {
        ...
        "modo_escrita": "overwrite",
        "escrito_por": "src/features/gold/model_enricher.py (ModelEnricher.write_profile_clusters_engagement), via scripts/run_profile_clustering_engagement.py",
        "lido_por": "pages/04_recommendations.py (comparação de perfil vs. pares do mesmo cluster)",
        "status": "Produção",
        ...
        "notas": "Roda pós-Gold-de-engajamento (lê governor_engagement) -- na Lambda serverless, é a etapa `model`, disparada depois de `load` (ver seção 3 do README).",
    },
    {
        "camada": "Gold",
        ...
        "lido_por": "pages/03_performance.py (comparação entre governadores, coeficientes da regressão)",
        "status": "Produção",
    ...
    },
    {
        ...
        "lido_por": "pages/03_performance.py (dispersão previsto vs. real, resíduos)",
        "status": "Produção",
    ...
    },
    {
        ...
        "lido_por": "pages/04_recommendations.py; pages/05_funil.py (priorização do que produzir a seguir)",
        "status": "Produção",
    ...
    },
    {
        ...
        "lido_por": "app.py (KPI 'NSM médio da base'); pages/05_funil.py",
    ...
    },
    {
        ...
        "caminho": "Sem caminho fixo em config/settings.py ainda -- GovernorUGCAggregator.write() exige path explícito do chamador, que ainda não existe em produção",
        "grao": "Uma linha por post de UGC (grão fino -- agregação por governador é uma view em memória, não persistida).",
        "modo_escrita": "overwrite (planejado)",
        "escrito_por": "Nenhum caminho de produção -- src/features/gold/ugc_mentions_aggregator.py (GovernorUGCAggregator) só é exercitado pelos testes (dado sintético)",
        "lido_por": "src/dashboard/filters.py (agregação, quando/se a tabela existir); pages/05_funil.py (nível 'Criar' do COBRA)",
        "status": "PILOTO -- mesma ressalva de Bronze/Silver ugc_mentions",
        "adr": "ADR 0020 Ficha 8 / issue #93",
        "descricao": "UGC orgânico vs. publi paga por post, com `is_organic` derivado de isPaidPartnership/isAd/isAffiliate -- separa apoio espontâneo de publi paga ANTES de qualquer agregação.",
        "notas": "`GovernorUGCAggregator.aggregate_by_governor` (a 'view' de contagem/engajamento médio por governador) nunca expõe `authorUsername` individual no agregado, por desenho -- privacidade de quem menciona o governador, não só do próprio governador.",
    },
    {
        "camada": "Gold",
        ...
        "lido_por": "pages/05_funil.py (Ficha 7 -- crescimento e retenção)",
...
COMMON_TECH: dict[str, str] = {
    ...
    "authorUsername": "Username de quem publicou o post de UGC (autor, distinto do governador mencionado).",
    "authorIsVerified": "Se o autor do post de UGC tem selo de verificação.",
    "isPaidPartnership": "Flag de parceria paga (publi) declarada pelo Instagram no post.",
    "isAd": "Flag de anúncio pago.",
    "isAffiliate": "Flag de conteúdo de afiliado.",
    "is_organic": "Derivado de isPaidPartnership/isAd/isAffiliate: True quando NENHuma das três é True -- separa apoio espontâneo de publi paga.",
    "governor_username": "Username do governador mencionado/marcado, resolvido a partir de `mentions` cruzado com a lista de perfis conhecidos.",
    "matchTypes": "Tipo de correlação do UGC ao perfil: marcação visual ('tagged') vs. menção textual ('mentioned').",
    "mentions": "Lista de usernames marcados/mencionados no post de UGC, serializada como JSON string.",
...
LINEAGE: list[dict] = [
    ...
    },
    {
        ...
        "modulo": "src/modeling/clustering.py + scripts/run_profile_clustering_engagement.py",
    },
    {
    ...
    {
        "origem": "Apify (apify/instagram-tagged-scraper) -- PILOTO",
        "destino": "Bronze/Silver/Gold: ugc_mentions",
        "transformacao": "Contrato de schema definido e testado com dado sintético; piloto real (scripts/run_apify_mentions_pilot.py) grava só em data/pilot/*.json, fora do Delta Lake -- pipeline de produção NÃO grava esta cadeia ainda.",
        "modulo": "src/features/silver/ugc_mention_cleaner.py + src/features/gold/ugc_mentions_aggregator.py",
    },
    {
        "origem": "Gold (todas as tabelas acima)",
        "destino": "Dashboards Streamlit (pages/01-05, app.py) e notebooks/03-07",
        ...
        "modulo": "src/repositories/delta_repository.py + src/dashboard/*",
    },
]
