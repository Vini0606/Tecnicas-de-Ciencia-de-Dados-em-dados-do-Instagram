...
TABLES: list[dict] = [
    ...
    {
        "camada": "Gold",
        "tabela": "governor_clusters_reels",
        "schema": sd.GOLD_CLUSTERS_SCHEMA,
        "caminho": "data/gold/governor_clusters_reels",
        "grao": "Uma linha por Reel -- clusterização de CONTEÚDO, não de perfil.",
        "modo_escrita": "overwrite",
        ...
        "status": "Produção",
        "adr": "ADR 0020 Ficha 2 / issue #87 (content_type); issue #152 (tabela separada de governor_clusters_posts)",
        "descricao": "Cluster de engajamento/duração por reel (PCA -> KMeans/DBSCAN/Agglomerative via AutoClusterHPO, algoritmo eleito por score CVI combinado).",
        "notas": "`cluster_label = -1` é ruído do DBSCAN, não um cluster de negócio (ver seção 2.2 do README) -- nunca a mesma coisa que clusterização de PERFIL (governor_profile_clusters_engagement, tabela separada). Antes da issue #152, coexistia com os posts de feed numa única tabela `governor_clusters` discriminada por `content_type` -- separada porque `posts_clean`/`reels_clean` (Silver) se sobrepõem (mesmo Reel capturado por dois actors Apify distintos), o que duplicava posts sob os dois `content_type`.",
    },
    {
        "camada": "Gold",
        "tabela": "governor_clusters_posts",
        "schema": sd.GOLD_CLUSTERS_SCHEMA,
        "caminho": "data/gold/governor_clusters_posts",
        "grao": "Uma linha por post de feed -- clusterização de CONTEÚDO, não de perfil.",
        "modo_escrita": "overwrite",
        "escrito_por": "src/features/gold/model_enricher.py (ModelEnricher.write_clusters), a partir de src/modeling/clustering.py (AutoClusterHPO)",
        "lido_por": "Nenhum consumidor de produção hoje (Tela de \"Padrões de conteúdo\" do feed ainda não existe no dashboard atual, ADR 0021).",
        "status": "Produção",
        "adr": "ADR 0020 Ficha 2 / issue #87 (content_type); issue #152 (tabela separada de governor_clusters_reels)",
        "descricao": "Cluster de engajamento por post de feed (PCA -> KMeans/DBSCAN/Agglomerative via AutoClusterHPO, algoritmo eleito por score CVI combinado).",
        "notas": "Mesmo schema de `governor_clusters_reels` (`content_type` constante = 'feed' dentro desta tabela) -- separada por causa da mesma sobreposição Silver descrita lá.",
    },
    {
        "camada": "Gold",
...
COMMON_TECH: dict[str, str] = {
    ...
    "content_type": "Valor constante por tabela ('reel' em governor_clusters_reels, 'feed' em governor_clusters_posts) -- mantido no schema das duas para permitir recombinação futura via pd.concat sem precisar re-derivar o rótulo (ADR 0020 Ficha 2 / issue #87; tabelas separadas desde a issue #152).",
...
LINEAGE: list[dict] = [
    ...
    },
    {
        ...
        "destino": "Gold: governor_clusters_reels + governor_clusters_posts",
        "transformacao": "PCA (engajamento/duração) + AutoClusterHPO (KMeans/DBSCAN/Agglomerative eleito por score CVI) por post; duas tabelas separadas (uma por formato, issue #152) em vez de uma combinada, porque reels_clean/posts_clean se sobrepõem (mesmo Reel capturado por dois actors Apify distintos).",
...
GLOSSARY: list[dict] = [
    ...
    {
        "metrica": "cluster_score (governor_clusters_reels / governor_clusters_posts / governor_profile_clusters_engagement)",
        "tabela": "governor_clusters_reels / governor_clusters_posts / governor_profile_clusters_engagement",
