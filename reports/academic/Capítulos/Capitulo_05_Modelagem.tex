...
\begin{figure}[h!]
    ...
    \label{fig:sentiment-top5-positivo}
\end{figure}
...
\begin{figure}[h!]
    ...
    \label{fig:sentiment-top5-negativo}
\end{figure}
...
\begin{figure}[h!]
    ...
    \label{fig:sentiment-cluster-m1}
\end{figure}
...
\subsection{Clusterização das Publicações do Feed}

% ROTEIRO (PENDENTE DE EXECUÇÃO):
% - Rodar AutoClusterHPO sobre as métricas de engajamento dos posts do FEED
%   (análogo ao que foi feito para os Reels).
% - Preencher: algoritmo eleito, hiperparâmetros, score CVI combinado,
%   número de clusters e o perfil de cada um (nº de posts, métricas médias).
% - Inserir figura de projeção dos clusters (análoga à \ref{fig:clusters_pca}).
% - Descrição APENAS: caracterizar os grupos. A leitura de growth fica no cap.6.
% - Comparar brevemente com os clusters de Reels: os padrões se repetem?

\subsection{Clusterização dos Perfis dos Governadores}

% ROTEIRO (PENDENTE DE EXECUÇÃO):
% - Rodar clusterização sobre as métricas AGREGADAS por perfil
%   (base da tabela \ref{tab:matriz_correlacao}).
% - Preencher: quantos grupos de governadores emergem, o que caracteriza
%   cada grupo (ex: alto alcance/baixa conversão), e quais governadores em cada.
% - Inserir figura/tabela dos grupos de perfis.
% - Descrição APENAS. A interpretação (segmentos à la Flesch) fica no cap.6.

\section{Modelagem de Tópicos com BERTopic}

A modelagem de tópicos identificou 50 temas distintos no corpus. As visualizações interativas fornecem insights profundos sobre a estrutura desses temas. O mapa da Figura~\ref{fig:intertopic} (Intertopic Distance Map) abaixo posiciona os tópicos em um espaço 2D com base em sua similaridade semântica. Cada círculo representa um tópico, e seu tamanho é proporcional à sua frequência.
...
\subsection{Modelagem de Tópicos do Discurso Oficial (Legendas e Transcrições)}

% ROTEIRO (PENDENTE DE EXECUÇÃO — legendas e transcrições vêm da API):
% - Rodar BERTopic (mesmo pipeline) sobre o texto produzido pelas assessorias:
%   legendas (captions) e transcrições da fala dos vídeos.
% - Preencher: nº de tópicos, temas dominantes, e as visualizações análogas
%   (mapa intertópico, dendrograma) para o discurso oficial.
% - Descrição APENAS dos temas do discurso. O CONTRASTE com os temas dos
%   comentários (o gap discurso x reação) é analisado no cap.6.

Os resultados apresentados neste capítulo \textemdash{} a segmentação das publicações por desempenho, a qualificação dos clusters pelo sentimento e a estrutura temática revelada pela modelagem de tópicos \textemdash{} constituem o diagnóstico bruto sobre o qual se assenta a etapa seguinte. No próximo capítulo, essas evidências deixam de ser descritas isoladamente e passam a ser organizadas segundo o funil de engajamento COBRA-RACE, traduzindo-se em métricas de crescimento e em recomendações práticas por perfil, de modo a converter a modelagem aqui desenvolvida em um instrumento efetivo de apoio à decisão para assessorias políticas.
