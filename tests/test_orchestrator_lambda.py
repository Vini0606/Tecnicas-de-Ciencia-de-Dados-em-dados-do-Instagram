...
def _set_function_names(monkeypatch):
    ...
    monkeypatch.setenv("MODEL_FUNCTION_NAME", "model-fn")


def test_orchestrator_encadeia_as_4_lambdas_e_propaga_run_id(monkeypatch):
    """extract -> transform -> load -> model (issue #86 / ADR 0020, Ficha 1):
    a clusterização de perfil de governador (Fase 2) passa a rodar
    automaticamente a cada execução do orquestrador, na posição pós-Gold-de-
    engajamento (`load`), de onde `governor_profile_clusters_engagement` lê
    `governor_engagement`."""
    ...
    responses = {
        ...
        "model-fn": {
            "statusCode": 200,
            "body": json.dumps(
                {"run_id": "generated-run", "status": "profile_clusters_engagement_complete"}
            ),
        },
    }
    invoked = []
    ...
    assert [name for name, _ in invoked] == ["extract-fn", "transform-fn", "load-fn", "model-fn"]
    # extract recebe os links e o run_id de entrada (None); transform/load/model
    # recebem o run_id que veio da resposta de extract, não o de entrada.
    ...
    assert invoked[3][1] == {"run_id": "generated-run"}


def test_orchestrator_aborta_se_model_falhar_mesmo_apos_load_ter_sucesso(monkeypatch):
    """Mesmo contrato de erro das outras etapas (issue #86): uma falha na
    etapa `model` interrompe a cadeia e reporta o stage certo, ainda que
    extract/transform/load já tenham persistido dado com sucesso."""
    from lambdas.orchestrator import handler as orchestrator_handler
    ...
    responses = {
        "extract-fn": {
            "statusCode": 200,
            "body": json.dumps({"run_id": "generated-run"}),
        },
        "transform-fn": {
            "statusCode": 200,
            "body": json.dumps({"run_id": "generated-run", "status": "silver_complete"}),
        ...
        "model-fn": {
            "statusCode": 500,
            "body": json.dumps({"stage": "model", "error": "Missing S3_BUCKET"}),
        },
    }
    invoked = []
    ...
    assert body["stage"] == "model"

    assert invoked == ["extract-fn", "transform-fn", "load-fn", "model-fn"]
